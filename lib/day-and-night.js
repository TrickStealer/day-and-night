...
export default {
  ...
  sunTimes() {
    ...
    // // Using solar-calc package instead
    // year = now.getFullYear()
    // // Month is zero indexed so need to add one to bring it inline with URL
    // month = now.getMonth() + 1
    // // Create the URL to retrieve the sunrise and sunset times
    // sunrise_sunset_api = "http://api.sunrise-sunset.org/json?lat="
    // // Use the user submitted GPS values for the API
    // sunrise_sunset_api = sunrise_sunset_api + lat + "&lng="
    // sunrise_sunset_api = sunrise_sunset_api + lng + "&formatted=0"
    // sunrise_sunset_api = sunrise_sunset_api + "&date=" + year + "-"
    // sunrise_sunset_api = sunrise_sunset_api + month + "-" + day
    //
    // console.log(sunrise_sunset_api)
    // this.download(sunrise_sunset_api).then((html) => {
    //     json = JSON.parse(html);
    //     console.log(json)
